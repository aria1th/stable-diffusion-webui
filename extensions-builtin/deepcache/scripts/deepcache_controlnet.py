...
class UNetHookModified(DeepCacheSession):
    ...
    session:Optional[DeepCacheSession] = None
    ...
    def patch_controlnet(params:DeepCacheParams, p:StableDiffusionProcessing):
        ...
        if shared.opts.deepcache_enable:
            ...
            UnetHook.hook = UNetHookModified.generate_vanilla_hook(params)
        return (UnetHook, original_hook, UNetHookModified.session) # return hook and original hook
    ...
    def generate_controlnet_hook(params:DeepCacheParams):
        ...
        """
        print("ControlNet hook generated")
        session = DeepCacheSession()
        session.load_params(params)
        UNetHookModified.session = session
        ...
        def get_hook(self:UnetHook, model, sd_ldm, control_params, process, batch_option_uint_separate=False, batch_option_style_align=False):
            """
            ...
            nonlocal session, cache_enable_step, full_run_step_rate, caching_level, CACHE_LAST, valid_cache_timestep_range
            self.model = model
            self.sd_ldm = sd_ldm
            ...
            def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
                nonlocal cnet_accumulate_timestep
                ...
                cnet_accumulate_timestep += 1
                if cnet_accumulate_timestep % 2 == 0:
                    session.enumerated_timestep['value'] += 1
                # U-Net Encoder
                ...
                with th.no_grad():
                    ...
                    print(f"Trying to load cache at timestep {session.enumerated_timestep['value']}")
                    cached_h = get_cache(session.enumerated_timestep["value"], real_timestep)

                    h = x
                    for i, module in enumerate(self.input_blocks):
                        print("ControlNet: input block", i)
                        self.current_h_shape = (h.shape[0], h.shape[1], h.shape[2], h.shape[3])
                        t2i_injection = [3, 5, 8] if is_sdxl else [2, 5, 8, 11]
                        if cached_h is not None and use_cache_cond() and i > caching_level:
                            print("ControlNet: input block", i, "skipped")
                            if i in t2i_injection:
                                total_t2i_adapter_embedding.pop(0)
                        else:
                            print("ControlNet: input block", i, "run")
                    ...
                    if not use_cache_cond():
                        print("ControlNet: middle block run")
                        self.current_h_shape = (h.shape[0], h.shape[1], h.shape[2], h.shape[3])
                        ...
                        print("ControlNet: middle block skipped")
                    relative_cache_level = len(self.output_blocks) - caching_level - 1
                
                # U-Net Decoder
                for i, module in enumerate(self.output_blocks):
                    if cached_h is not None and use_cache_cond() and i == relative_cache_level:
                        print("ControlNet: output block", i, "loaded cache")
                        # use cache
                        h = cached_h
                    elif cache_cond() and i == relative_cache_level:
                        print("ControlNet: output block", i, "put cache")
                        # put cache
                        put_cache(h, session.enumerated_timestep["value"], real_timestep)
                    elif cached_h is not None and use_cache_cond() and i < relative_cache_level:
                        print("ControlNet: output block", i, "skipping")
                        total_controlnet_embedding.pop()
                        continue
                    print("ControlNet: output block", i, "run")
                    self.current_h_shape = (h.shape[0], h.shape[1], h.shape[2], h.shape[3])
        ...
        return get_hook
    @staticmethod
    def generate_vanilla_hook(params:DeepCacheParams):
        print("ControlNet: generate_vanilla_hook")
        def hook(self, model, sd_ldm, control_params, process, batch_option_uint_separate=False, batch_option_style_align=False):
            print("ControlNet: hook as vanilla")
